...
const cuentas: Account[] = [
  { id: 'op', nombre: 'Corriente operativa', banco: 'Popular', tipo: 'corriente', ultimos4: '2233', moneda: 'DOP' },
  { id: 'pr', nombre: 'Corriente principal', banco: 'Popular', tipo: 'corriente', ultimos4: '5566', moneda: 'DOP' },
  { id: 'tc', nombre: 'Tarjeta', banco: 'Popular', tipo: 'tarjeta', ultimos4: '9090', moneda: 'DOP', limite: 150000 },
]
...
describe('traslados entre cuentas propias', () => {
  it('empareja los dos lados de una transferencia entre cuentas', () => {
    const txns = [
      t('s1', 'op', '2026-05-24', 'Transf. via MB a 700445566', -30),
      t('e1', 'pr', '2026-05-24', 'Transf. via MB desde 700112233', 30),
    ]
  ...
  it('empareja un pago de tarjeta con su abono, aunque se llamen distinto', () => {
    // La cuenta lo llama "PagoTC Via MB***9090"; la tarjeta lo llama "Pago Via App".
    const txns = [
      t('s1', 'op', '2026-07-30', 'PagoTC Via MB***9090', -42150.75),
      t('e1', 'tc', '2026-07-30', 'Pago Via App', 42150.75),
    ]
    const { enlaces } = emparejarTraslados(txns, cuentas)
    expect(enlaces).toHaveLength(1)
    expect(enlaces[0].metodo).toBe('pago-tarjeta')
    expect(enlaces[0].monto).toBe(42150.75)
  })

  it('tolera que la tarjeta acredite el pago un par de dias despues', () => {
    const txns = [
      t('s1', 'op', '2026-08-07', 'PagoTC Via MB***9090', -40000),
  ...
  it('NO empareja un pago real a un tercero por el mismo monto', () => {
    const txns = [
      t('s1', 'op', '2026-06-01', 'MB a 0790206593 J RODRIGUEZ', -15000),
      t('e1', 'pr', '2026-06-01', 'RENTA LOCAL COMERCIAL', 15000),
    ]
  ...
  it('no usa dos veces la misma entrada cuando hay montos repetidos', () => {
    const txns = [
      t('s1', 'op', '2026-06-27', 'Transf. via MB a 700445566', -11000),
      t('s2', 'op', '2026-06-30', 'Transf. via MB a 700445566', -11000),
      t('e1', 'pr', '2026-06-27', 'Transf. via MB desde 700112233', 11000),
      t('e2', 'pr', '2026-06-30', 'Transf. via MB desde 700112233', 11000),
    ]
    const { enlaces } = emparejarTraslados(txns, cuentas)
  ...
  it('empareja una tanda completa como la de un estado real', () => {
    ...
    const txns: Txn[] = []
    montos.forEach((m, i) => {
      txns.push(t(`s${i}`, 'op', fechas[i], 'Transf. via MB a 700445566', -m))
      txns.push(t(`e${i}`, 'pr', fechas[i], 'Transf. via MB desde 700112233', m))
    })
    const { enlaces } = emparejarTraslados(txns, cuentas)
...
describe('totales limpios de traslados', () => {
  it('saca el traslado del gasto y lo reporta aparte', () => {
    const txns: Txn[] = [
      ...
      { ...t('c', 'op', '2026-08-07', 'PagoTC Via MB***9090', -40000), esInterno: true },
